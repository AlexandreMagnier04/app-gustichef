<script lang="ts">
	...
	const recapTitle = $derived([
		menuTitle,
		...selectedExtras.map(e => e.title)
	].join(' + '));
...
<div class="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
	<div class="w-full max-w-lg overflow-hidden rounded-t-2xl bg-white" style="max-height: 92dvh; overflow-y: auto;">
		...
		{#if step === 'success'}
			...
			<div class="flex flex-col items-center px-5 pt-8 pb-10 text-center">
				...
				<span class="mt-3 rounded-full bg-amber-100 px-3 py-1 text-[12px] font-semibold text-amber-700">En attente</span>

				<div class="mt-6 w-full rounded-2xl border border-navy/8 bg-white p-4 text-left">
					<p class="text-xs font-semibold tracking-wide text-navy/40 uppercase">Votre réservation</p>
					<p class="mt-2 text-sm font-semibold text-navy">{recapTitle}</p>
					<p class="mt-0.5 text-[12px] capitalize text-navy/60">{formatDateFr(eventDate)}{eventTime ? ' · ' + eventTime : ''}</p>
					<p class="mt-0.5 text-[12px] text-navy/60">{guests} convive{guests > 1 ? 's' : ''}</p>
					<p class="mt-0.5 text-[12px] text-navy/60">{localization} · Frais de déplacement inclus</p>
				</div>

				<div class="mt-4 w-full space-y-3">
					<button
						type="button"
						onclick={() => { if (convId) onsuccess(convId); }}
		...
		{:else}
			...
			<div class="flex items-center justify-between px-5 py-3">
				...
				<button type="button" onclick={onclose} class="flex h-8 w-8 items-center justify-center rounded-full bg-navy/8 text-navy/50">
			...
			<div class="flex justify-center gap-1.5 pb-4">
				{#each [1, 2, 3] as s}
					<div class="h-1.5 rounded-full transition-all {step >= s ? 'w-6 bg-rust' : 'w-1.5 bg-navy/20'}"></div>
				{/each}
			</div>
			...
			<div class="px-5 pb-8">
				{#if step === 1}
					...
					<div class="space-y-4">
						<div>
							<label class="mb-1.5 block text-[12px] font-semibold text-navy/60">Date de l'événement</label>
							<DatePicker bind:value={eventDate} min={new Date().toISOString().split('T')[0]} placeholder="Choisir la date" />
						</div>
						<div>
							<label class="mb-1.5 block text-[12px] font-semibold text-navy/60">Heure de début</label>
							<input
								type="time"
						...
						<div class="relative">
							...
							{#if showLocSuggestions}
								<ul class="absolute top-full left-0 z-20 mt-1 w-full overflow-hidden rounded-xl border border-gray-100 bg-white shadow-lg">
				...
				{:else if step === 2}
					...
					<div class="overflow-hidden rounded-xl border border-navy/8 bg-white">
						...
						<div class="p-3">
							...
							<p class="text-[12px] text-rust">Menu sélectionné · Dès {pricePerPerson} € / convive</p>
						</div>
					</div>
					...
					<div class="mt-4">
						...
						<div class="flex overflow-hidden rounded-xl border border-navy/10">
							<button
								type="button"
								onclick={() => { if (guests > guestsMin) guests--; }}
							...
							>−</button>
							<span class="flex flex-1 items-center justify-center bg-cream text-xl font-bold text-navy">{guests}</span>
							<button
								type="button"
								onclick={() => { if (guests < guestsMax) guests++; }}
							...
							<div class="space-y-2">
								{#each chiefExtras as extra (extra.id_menu)}
									<div class="flex items-center gap-3 rounded-xl border border-navy/8 bg-white p-3">
										<div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-cream text-2xl">🍽</div>
										...
										<div class="flex items-center gap-2">
											<button
												type="button"
												onclick={() => { const q = extrasQty[extra.id_menu] ?? 0; if (q > 0) extrasQty = { ...extrasQty, [extra.id_menu]: q - 1 }; }}
											...
											<button
												type="button"
												onclick={() => { const q = extrasQty[extra.id_menu] ?? 0; extrasQty = { ...extrasQty, [extra.id_menu]: q + 1 }; }}
					...
					<div class="mt-4 rounded-xl border border-navy/8 bg-white px-4 py-3">
						...
						{#each chiefExtras.filter(e => (extrasQty[e.id_menu] ?? 0) > 0) as extra}
							<div class="mt-1 flex justify-between text-[13px] text-navy/60">
								<span>{extra.title_menu} ({guests} × {extra.price_menu} €)</span>
								<span>{(parseFloat(extra.price_menu ?? '0') * (extrasQty[extra.id_menu] ?? 0) * guests).toFixed(0)} €</span>
							</div>
						{/each}
						<div class="mt-1 flex justify-between text-[13px] text-navy/60">
						...
						<div class="mt-2 flex justify-between border-t border-navy/8 pt-2 text-sm font-bold text-navy">
					...
					<div class="mt-6 flex gap-3">
						<button type="button" onclick={() => step = 1} class="flex-1 rounded-xl border border-navy/15 py-3.5 text-sm font-medium text-navy">Retour</button>
						<button type="button" onclick={() => step = 3} class="flex-1 rounded-xl bg-navy py-3.5 text-sm font-semibold text-white">Continuer</button>
					</div>
				...
				{:else if step === 3}
					...
					<div class="space-y-4">
						<div>
							<label class="mb-1.5 block text-[13px] font-semibold text-navy">Notes ou préférences</label>
							<textarea
								bind:value={notes}
						...
						<div class="rounded-xl border border-navy/8 bg-white p-4">
							<p class="text-[12px] font-semibold tracking-wide text-navy/40 uppercase">Récapitulatif de votre réservation</p>
							<div class="mt-2 space-y-1 text-[13px] text-navy/70">
								<p class="capitalize">{formatDateFr(eventDate)}{eventTime ? ' · ' + eventTime : ''}</p>
							...
							<div class="mt-3 space-y-1 border-t border-navy/8 pt-3">
								...
								<div class="flex justify-between border-t border-navy/8 pt-2 text-sm font-bold text-navy">
									<span>Total estimé</span>
									<span>{grandTotal.toFixed(1)} €</span>
								</div>
							</div>
							<p class="mt-2 text-[11px] text-navy/40">* Ce tarif est une estimation. Le prix final sera validé par le chef</p>
						</div>
					</div>

					<div class="mt-6 flex gap-3">
						<button type="button" onclick={() => step = 2} class="flex-1 rounded-xl border border-navy/15 py-3.5 text-sm font-medium text-navy">Retour</button>
						<button
							type="button"
							onclick={submit}
							disabled={submitting}
