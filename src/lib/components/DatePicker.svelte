...
<div class="relative">
	...
	<button
		type="button"
		onclick={() => (open = !open)}
		class="flex w-full items-center justify-between rounded-xl border bg-white px-4 py-3 text-left text-sm transition-colors {open ? 'border-rust' : 'border-navy/15'}"
	>
		{#if displayValue}
			<span class="capitalize text-navy">{displayValue}</span>
		{:else}
			<span class="text-navy/30">{placeholder}</span>
		{/if}
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4 shrink-0 text-navy/40">
	...
	<!-- Calendrier -->
	{#if open}
		<div class="absolute left-0 right-0 z-50 mt-2 overflow-hidden rounded-2xl border border-navy/10 bg-white shadow-xl">
			<!-- Navigation mois -->
					...
					{:else}
						...
						<button
							...
							class="flex h-9 w-full items-center justify-center rounded-xl text-[13px] font-medium transition-colors
								{sel ? 'bg-rust text-white font-semibold' : ''}
...
{#if open}
	<button class="fixed inset-0 z-40" onclick={() => (open = false)} aria-label="Fermer le calendrier" tabindex="-1"></button>
{/if}
