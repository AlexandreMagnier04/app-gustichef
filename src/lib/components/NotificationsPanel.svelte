	...
	>
		...
		<div class="flex items-center justify-between px-5 pb-3">
			...
			{#if unreadCount > 0}
				<button
					type="button"
					onclick={markAllRead}
					class="text-xs font-medium text-teal underline"
				>
		...
		{#if loading}
			<div class="flex items-center justify-center py-12">
				<div class="h-6 w-6 animate-spin rounded-full border-2 border-navy/20 border-t-navy/60"></div>
			</div>
		{:else if notifs.length === 0}
			<div class="flex flex-col items-center justify-center px-8 py-14 text-center">
				<div class="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-navy/5">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-7 w-7 text-navy/30">
