...
<article class="overflow-hidden bg-white shadow-[0_1px_6px_rgba(22,48,64,0.06)]">
	...
	<div class="relative aspect-[4/3] w-full">
		...
		<div class="pointer-events-none absolute inset-x-0 bottom-0 h-2/5 bg-linear-to-t from-black/55 via-black/15 to-transparent"></div>
		...
		>
			{#if publication.author.image}
				...
				/>
			{:else}
				<div class="flex h-8 w-8 items-center justify-center rounded-full border border-white/80 bg-cream/90 text-navy/40">
			...
			<span class="truncate text-sm font-semibold text-white drop-shadow-sm">
				{publication.author.firstname} {publication.author.name}
			</span>
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="ml-0.5 h-3 w-3 shrink-0 text-white/80">
		...
		{#if visibleTags.length > 0}
			<div class="flex flex-wrap gap-1.5">
				{#each visibleTags as t (t.label + t.kind)}
					{#if t.kind === 'category'}
						<span class="rounded-full bg-rust/90 px-2.5 py-1 text-[10px] font-medium tracking-wide text-white">
							{t.label}
						</span>
					{:else}
						<span class="rounded-full bg-emerald-700/85 px-2.5 py-1 text-[10px] font-medium tracking-wide text-white">
							{t.label}
						</span>
					{/if}
				{/each}
