<script lang="ts">
	...
	async function submit(e: Event) {
		...
		const res = await fetch(`/api/requests/${request.id_request}/respond`, {
			...
			body: JSON.stringify({ message: message.trim(), price_per_person: price ? Number(price) : null }),
		});
		...
		<div class="px-6 pt-2">
			{#if success}
				<div class="flex flex-col items-center py-8 text-center">
					...
					<p class="mt-2 text-sm leading-relaxed text-navy/60">Votre message a été transmis au client</p>
					<button
						type="button"
						onclick={close}
			...
			{:else}
				...
				<form onsubmit={submit} class="flex flex-col gap-5">
					...
					<div>
						<label class="mb-1.5 block text-sm font-medium text-navy">Prix proposé / convive <span class="text-navy/40">(optionnel)</span></label>
						<div class="relative">
							<select
								bind:value={price}
					...
					<button
						type="submit"
						disabled={submitting}
						class="flex w-full items-center justify-center gap-2 rounded-xl bg-rust py-3.5 text-sm font-semibold text-white shadow-sm transition-opacity disabled:opacity-60 active:opacity-80"
					>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4">
