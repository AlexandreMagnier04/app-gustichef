<script lang="ts">
	...
	async function validate() {
		if (!stripe || !cardElement) { cardError = 'Stripe non chargé'; return; }
		if (!data.stripeClientSecret) { cardError = 'Session expirée, rechargez la page'; return; }
		cardError = '';
		submitting = true;
		...
		submitting = false;
		if (!res.ok) { cardError = 'Erreur lors de la validation. Réessayez.'; return; }
		succeeded = true;
	}
</script>
...
<div class="-mx-5 -mt-3 min-h-[calc(100dvh-120px)] bg-white">
	...
	<div class="flex shrink-0 items-center gap-3 border-b border-navy/8 bg-white px-4 py-3">
		<a href="/messages/{conv.id_conversation}" aria-label="Retour" class="flex h-9 w-9 items-center justify-center rounded-full bg-white shadow-sm">
	...
	<div class="space-y-4 px-4 py-4 pb-40">
		...
		<div class="overflow-hidden rounded-2xl border border-navy/8 bg-white">
			{#if menuImage}
				<img src={menuImage} alt={lastProposal?.menu_title ?? 'Menu'} class="h-44 w-full object-cover" />
			{:else}
				<div class="flex h-32 items-center justify-center bg-navy/5">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-10 w-10 text-navy/20">
			...
			<div class="p-4">
				...
				{#if lastProposal?.menu_description}
					<p class="mt-1 line-clamp-2 text-[13px] leading-snug text-navy/60">{lastProposal.menu_description}</p>
				{/if}
				{#if effectivePrice}
		...
		<!-- Récap -->
		<div class="overflow-hidden rounded-2xl border border-navy/8 bg-white">
			<div class="bg-navy/5 px-4 py-2 text-[11px] font-semibold tracking-wide text-navy/50 uppercase">Récapitulatif</div>
			<div class="divide-y divide-navy/5 px-4">
				{#if conv.request_date}
					<div class="flex items-center justify-between py-3">
						<span class="text-[13px] text-navy/60">Date</span>
						<span class="text-[13px] font-medium capitalize text-navy">{formatDate(conv.request_date)}</span>
					</div>
				{/if}
				<div class="flex items-center justify-between py-3">
					<span class="text-[13px] text-navy/60">Heure</span>
					<input type="time" bind:value={eventTime} class="rounded-lg border border-navy/15 bg-white px-2 py-1 text-[13px] font-medium text-navy outline-none focus:border-navy" />
				</div>
		...
		{#if chiefExtras.length > 0}
			<div class="overflow-hidden rounded-2xl border border-navy/8 bg-white">
				<div class="bg-navy/5 px-4 py-2 text-[11px] font-semibold tracking-wide text-navy/50 uppercase">Compléments <span class="font-normal normal-case">(optionnel)</span></div>
					...
					{#each chiefExtras as extra (extra.id_menu)}
						<div class="flex items-center gap-3 px-4 py-3">
							...
							<div class="flex items-center gap-2">
								<button type="button" onclick={() => { const q = extrasQty[extra.id_menu] ?? 0; if (q > 0) extrasQty = { ...extrasQty, [extra.id_menu]: q - 1 }; }} disabled={(extrasQty[extra.id_menu] ?? 0) === 0} class="flex h-7 w-7 items-center justify-center rounded-full bg-navy/8 text-sm font-bold text-navy disabled:opacity-30">−</button>
								<span class="w-5 text-center text-[13px] font-semibold text-navy">{extrasQty[extra.id_menu] ?? 0}</span>
								<button type="button" onclick={() => { const q = extrasQty[extra.id_menu] ?? 0; extrasQty = { ...extrasQty, [extra.id_menu]: q + 1 }; }} class="flex h-7 w-7 items-center justify-center rounded-full bg-navy/8 text-sm font-bold text-navy">+</button>
							</div>
						</div>
					{/each}
				</div>
		...
		<div class="overflow-hidden rounded-2xl border border-navy/8 bg-white">
			<div class="divide-y divide-navy/5 px-4">
				...
				{#each chiefExtras.filter((e: ExtraMenu) => (extrasQty[e.id_menu] ?? 0) > 0) as extra (extra.id_menu)}
					<div class="flex justify-between py-2.5">
						<span class="text-[13px] text-navy/70">{extra.title_menu} (×{extrasQty[extra.id_menu]})</span>
						<span class="text-[13px] font-medium text-navy">{(parseFloat(extra.price_menu ?? '0') * (extrasQty[extra.id_menu] ?? 0) * guests).toFixed(0)} €</span>
					</div>
				{/each}
				<div class="flex justify-between py-2.5">
			...
			<p class="px-4 pb-3 text-[11px] text-navy/40">Ce tarif est une estimation. Le prix final sera validé par le chef.</p>
		</div>

		<!-- Empreinte carte -->
		<div class="overflow-hidden rounded-2xl border border-navy/8 bg-white">
			<div class="bg-navy/5 px-4 py-2 text-[11px] font-semibold tracking-wide text-navy/50 uppercase">Empreinte carte bancaire</div>
			<div class="space-y-4 p-4">
				...
				<div class="flex items-start gap-2.5 rounded-xl bg-teal/8 px-3 py-2.5">
					...
					<p class="text-[12px] text-teal">Comment fonctionne le paiement ? <strong>Aucun débit aujourd'hui</strong> — votre CB sécurise uniquement la réservation.</p>
				</div>
				...
				<div class="space-y-3">
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<p class="text-[12px] text-navy/60">Acompte de 50 % débité — <strong class="text-navy">{(grandTotal * 0.5).toFixed(0)} €</strong> pour l'achat des ingrédients</p>
						</div>
					</div>
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<p class="text-[12px] text-navy/60">Solde de 50 % débité — <strong class="text-navy">{(grandTotal * 0.5).toFixed(0)} €</strong> une fois la prestation confirmée</p>
						</div>
					</div>
				</div>
				...
				<div class="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
					...
					<p class="text-[11px] text-amber-700">
						<span class="font-semibold">Politique d'annulation :</span> Annulation gratuite jusqu'à J-7. Acompte non remboursable après J-7. En revanche, vous acceptez les <span class="underline">conditions générales de Gustichef</span>.
					</p>
				</div>
				...
				<div>
					...
					<div id="stripe-card-element" class="rounded-xl border border-navy/15 bg-white px-3 py-3.5"></div>
					<p class="mt-1 text-[11px] text-navy/40">Carte test : 4242 4242 4242 4242 — date future — CVV quelconque</p>
				</div>
				...
				<!-- SSL -->
				<div class="flex items-center justify-center gap-1.5 rounded-xl border border-teal/30 bg-teal/5 py-2 text-[11px] font-medium text-teal">
	...
	<div class="fixed right-0 bottom-16 left-0 z-40 border-t border-navy/8 bg-white px-4 py-3">
		<button type="button" onclick={validate} disabled={submitting} class="w-full rounded-xl bg-navy py-4 text-sm font-semibold text-white transition-opacity active:opacity-80 disabled:opacity-50">
	...
	<div class="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
		<div class="w-full max-w-lg overflow-hidden rounded-t-3xl bg-white pb-8">
			...
			<div class="px-6 text-center">
				<div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-teal/15">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-7 w-7 text-teal">
				...
				<p class="mt-1.5 text-[13px] leading-relaxed text-navy/60">
					Le chef <span class="font-medium text-navy">{conv.other_firstname}</span> va étudier votre demande et vous répondra sous 24h.
				</p>
				<div class="mt-3 inline-flex items-center gap-1.5 rounded-full border border-navy/20 px-3 py-1 text-[12px] font-medium text-navy/60">
					<span class="h-1.5 w-1.5 rounded-full bg-amber-400"></span>
					En attente
				</div>
				<div class="mt-4 rounded-2xl border border-navy/8 bg-cream p-4 text-left">
					<p class="text-[11px] font-semibold tracking-wide text-navy/40 uppercase">Votre réservation</p>
					<p class="mt-2 text-sm font-bold text-navy">{lastProposal?.menu_title ?? 'Prestation'}</p>
					{#if conv.request_date}
						<p class="mt-0.5 text-[12px] capitalize text-navy/60">{formatDate(conv.request_date)}{eventTime ? ' à ' + eventTime : ''}</p>
					{/if}
				...
				<div class="mt-4 space-y-2.5">
					<button type="button" onclick={() => goto('/customer-profile')} class="w-full rounded-xl bg-navy py-3.5 text-sm font-semibold text-white transition-opacity active:opacity-80">
						Voir mes réservations
					</button>
					<button type="button" onclick={() => goto(`/messages/${conv.id_conversation}`)} class="w-full rounded-xl bg-rust py-3.5 text-sm font-semibold text-white transition-opacity active:opacity-80">
						Écrire au chef
					</button>
				</div>
			</div>
