<script lang="ts">
	...
	function statusBadge(statut: string): { label: string; cls: string } {
		switch (statut) {
			...
			default:
				return { label: 'En attente', cls: 'bg-orange-50 text-orange-600 border border-orange-200' };
		}
	}
...
<div class="mb-5">
	<div class="mb-4 flex items-start gap-4">
		...
		<div class="relative shrink-0">
			...
			{/if}
			<button
				class="absolute bottom-0 right-0 flex h-6 w-6 items-center justify-center rounded-full bg-rust text-white shadow"
				aria-label="Modifier la photo"
			>
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="h-3.5 w-3.5">
					<path d="M13.488 2.513a1.75 1.75 0 0 0-2.475 0L6.75 6.774a2.75 2.75 0 0 0-.596.892l-.848 2.047a.75.75 0 0 0 .98.98l2.047-.848a2.75 2.75 0 0 0 .892-.596l4.261-4.262a1.75 1.75 0 0 0 0-2.474ZM4.75 13.25a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5Z" />
				</svg>
			</button>
		</div>
...
{#if activeTab === 'reservations'}
	...
	<div class="mb-5 flex rounded-xl bg-navy/8 p-1">
		<button
			...
			class="flex-1 rounded-lg py-2.5 text-sm font-semibold transition-colors {reservationFilter === 'upcoming'
		...
		<button
			...
			class="flex-1 rounded-lg py-2.5 text-sm font-semibold transition-colors {reservationFilter === 'past'
				? 'bg-navy text-cream shadow-sm'
				: 'text-navy/50'}"
		>
			Passées
	...
	{#if activeList.length === 0}
		<div class="flex flex-col items-center gap-3 py-12 text-navy/40">
			...
			<p class="text-sm">Aucune réservation {reservationFilter === 'upcoming' ? 'à venir' : 'passée'}</p>
		</div>
	{:else}
				...
				<div class="overflow-hidden rounded-2xl bg-white shadow-[0_2px_8px_rgba(22,48,64,0.08)]">
						...
						<div class="flex items-center gap-3">
							...
							<div>
								<div class="flex items-center gap-1">
									<span class="text-sm font-semibold text-navy">{reservation.chief_firstname} {reservation.chief_name}</span>
					...
					<div class="grid grid-cols-2 gap-x-3 gap-y-2 px-4 py-3">
						...
						<div class="flex items-center gap-2 text-xs text-navy/60">
							...
							<span>{reservation.guests_request} convive{reservation.guests_request > 1 ? 's' : ''}</span>
						</div>
						{#if reservation.type_event_request}
							<div class="flex items-center gap-2 text-xs text-navy/60">
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="h-3.5 w-3.5 shrink-0 text-navy/30">
									<path d="M5.5 1a.5.5 0 0 0-1 0v1H4a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2h-.5V1a.5.5 0 0 0-1 0v1h-5V1Z" />
								</svg>
					...
					<div class="flex gap-2 px-4 pb-4">
						...
						<button
							class="flex-1 rounded-xl bg-navy py-2.5 text-xs font-semibold text-cream"
						>
							Contacter
						</button>
					</div>
