<script lang="ts">
	...
	let respondingRequest = $state<import('$lib/server/services/customers').RequestWithCustomer | null>(null);
	$effect(() => { if (!showNewRequest) editingRequest = null; });
	...
	const upcomingRequests = $derived(customerRequests.filter((r) => r.expected_date_request >= today));
...
{:else}
	...
	{#if isChief}
		<!-- Filtre ville -->
		<div class="-mx-5 flex items-center gap-3 border-b border-navy/[0.07] px-4 py-3">
			{#if requestCityFilter}
				<span class="inline-flex items-center gap-2 rounded-full border border-teal bg-[#FDF7F4] px-4 py-1.5 text-sm text-navy/80">
					{requestCityFilter}
					<button type="button" onclick={() => (requestCityFilter = '')} aria-label="Effacer" class="text-navy/40">
			...
			{:else}
				<input
					...
					class="flex-1 rounded-full border border-navy/15 bg-white px-3 py-1.5 text-sm text-navy placeholder:text-navy/30 outline-none focus:border-teal"
				/>
			{/if}
		</div>

		<div class="-mx-5 flex flex-col gap-2.5 pt-2.5 pb-10">
			{#each filteredRequests as r (r.id_request)}
				<article class="mx-4 overflow-hidden rounded-2xl border border-navy/[0.07] bg-white shadow-sm">
					<div class="p-4">
						<!-- Header : titre + client -->
						<div class="flex items-start justify-between gap-3">
							<h3 class="text-[15px] font-semibold leading-snug text-navy">{r.title_request}</h3>
						...
						<!-- Description -->
						<p class="mt-2.5 line-clamp-3 text-[13px] leading-snug text-navy/65">{r.description_request}</p>

						<!-- CTA -->
						<button
							type="button"
							onclick={() => { respondingRequest = r; showRespondModal = true; }}
						...
						>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4">
								<path fill-rule="evenodd" d="M10 2c-2.236 0-4.43.18-6.57.524C1.993 2.755 1 4.014 1 5.426v5.148c0 1.413.993 2.67 2.43 2.902.848.137 1.705.248 2.57.331v3.443a.75.75 0 0 0 1.28.53l3.58-3.579a.78.78 0 0 1 .527-.224 41.202 41.202 0 0 0 5.183-.5c1.437-.232 2.43-1.49 2.43-2.903V5.426c0-1.413-.993-2.67-2.43-2.902A41.289 41.289 0 0 0 10 2Zm0 7a1 1 0 1 0 0-2 1 1 0 0 0 0 2ZM6 9a1 1 0 1 0 0-2 1 1 0 0 0 0 2Zm8-1a1 1 0 1 0-2 0 1 1 0 0 0 2 0Z" clip-rule="evenodd" />
							</svg>
	...
	<!-- ===== DEMANDES — VUE CLIENT ===== -->
	{:else}
		<div class="-mx-5 flex flex-col pb-10">
			{#if customerRequests.length === 0}
				<div class="flex flex-col items-center justify-center px-8 py-16 text-center">
					...
					<p class="mt-1 text-sm text-navy/50">Appuyez sur + pour envoyer votre première demande !</p>
				</div>
			{:else}
				...
				<button
					...
					class="mx-auto flex items-center gap-3 py-4 transition-opacity active:opacity-70"
				>
					<span class="flex h-10 w-10 items-center justify-center rounded-full bg-teal text-white">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5">
							<path d="M10.75 4.75a.75.75 0 0 0-1.5 0v4.5h-4.5a.75.75 0 0 0 0 1.5h4.5v4.5a.75.75 0 0 0 1.5 0v-4.5h4.5a.75.75 0 0 0 0-1.5h-4.5v-4.5Z" />
						</svg>
					</span>
	...
	<article class="overflow-hidden rounded-2xl border border-navy/[0.07] bg-white shadow-sm">
		<div class="p-4">
			...
			<div class="flex items-start justify-between gap-3">
				...
				{#if editable}
					<div class="flex shrink-0 flex-col items-end gap-0.5">
						<span class="text-xs font-medium text-navy/70">{data.user.firstname} {data.user.name}</span>
						<button type="button" class="text-[11px] text-rust underline" onclick={() => { editingRequest = r; showNewRequest = true; }}>modifier demande</button>
					</div>
				{/if}
			</div>
			...
			<div class="mt-2.5 flex flex-wrap items-center gap-1.5">
				<span class="inline-flex items-center gap-1 rounded-full bg-olive px-2.5 py-1 text-[11px] text-white">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="h-3 w-3 opacity-70">
						<path fill-rule="evenodd" d="M4 1.75a.75.75 0 0 1 1.5 0V3h5V1.75a.75.75 0 0 1 1.5 0V3A2.5 2.5 0 0 1 14.5 5.5v6a2.5 2.5 0 0 1-2.5 2.5H4A2.5 2.5 0 0 1 1.5 11.5v-6A2.5 2.5 0 0 1 4 3V1.75ZM3 7v4.5A1 1 0 0 0 4 12.5h8a1 1 0 0 0 1-1V7H3Z" clip-rule="evenodd" />
					</svg>
					{formatDate(r.expected_date_request)}
				</span>
