<script lang="ts">
	...
	const TAB_STATUT: Record<string, string | null> = isChief
		? { 'Tous': null, 'À répondre': 'a_repondre', 'Devis envoyé': 'devis_envoye', 'Confirmé': 'confirme' }
		: { 'Tous': null, 'En attente': 'a_repondre', 'Devis reçu': 'devis_envoye', 'Confirmé': 'confirme' };
	...
	const filtered = $derived(
		activeTab === 'Tous'
			...
			: (data.conversations as ConversationListItem[]).filter((c) => c.statut === TAB_STATUT[activeTab])
	);
	...
	function formatTime(d: string | Date): string {
		...
		if (diff < 604800000) return ['dim.', 'lun.', 'mar.', 'mer.', 'jeu.', 'ven.', 'sam.'][date.getDay()];
	...
	<div class="scrollbar-none flex gap-2 overflow-x-auto border-b border-navy/[0.07] px-4 pt-2 pb-0">
		{#each TABS as tab (tab)}
			<button
				...
				class="shrink-0 border-b-2 px-3 pb-2.5 text-sm font-medium transition-colors {activeTab === tab
	...
	{#if filtered.length === 0}
		<div class="flex flex-col items-center justify-center px-8 py-20 text-center">
			<div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-navy/5">
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-8 w-8 text-navy/30">
	...
	{:else}
		<ul class="divide-y divide-navy/[0.06]">
			{#each filtered as conv (conv.id_conversation)}
				{@const statut = STATUT_LABELS[conv.statut] ?? { label: conv.statut, color: 'bg-navy/10 text-navy/50' }}
				<li>
					<a href="/messages/{conv.id_conversation}" class="flex items-start gap-3 px-4 py-3.5 transition-colors hover:bg-navy/[0.02] active:bg-navy/[0.04]">
						<!-- Avatar -->
						{#if conv.other_image}
							<img src={conv.other_image} alt="" class="h-12 w-12 shrink-0 rounded-full object-cover" />
						{:else}
							<div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-cream text-navy/40 text-lg font-semibold">
						...
						<div class="min-w-0 flex-1">
							<div class="flex items-baseline justify-between gap-2">
								<span class="truncate text-[14px] font-semibold text-navy">{conv.other_firstname} {conv.other_name}</span>
								<span class="shrink-0 text-[11px] text-navy/40">{formatTime(conv.last_message_at)}</span>
							</div>
							...
							<!-- Statut badge -->
							<span class="mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold {statut.color}">
								{statut.label}
							</span>
						...
						{#if conv.unread_count > 0}
							<span class="mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-rust text-[10px] font-bold text-white">
								{conv.unread_count}
							</span>
						{/if}
					</a>
