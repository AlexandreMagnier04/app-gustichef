<script lang="ts">
	...
	const menuItems = [
		{ icon: 'user',      label: 'Informations personnelles', sub: 'Nom, photo de profil',  href: '/profile/settings/infos' },
		{ icon: 'check',     label: 'Connexion & sécurité',      sub: 'Mot de passe, 2FA',     href: '/profile/settings/security' },
		{ icon: 'bell',      label: 'Notifications',              sub: 'Push, email, SMS',      href: '#' },
		{ icon: 'money',     label: 'Paiements & conditions',     sub: 'Cartes, annulation',    href: '#' },
		{ icon: 'file',      label: "Conditions d'utilisation",   sub: 'CGU, RGPD',             href: '/profile/settings/cgu' }
	];
</script>

<!-- En-tête -->
<div class="flex items-center justify-between pb-4 pt-2">
	<a href={backHref} class="text-navy/50 hover:text-navy">
...
<div class="mb-4 flex items-center gap-3 rounded-2xl border border-navy/8 px-4 py-3">
	...
	{:else}
		<div class="flex h-12 w-12 items-center justify-center rounded-full bg-navy/10 text-sm font-bold text-navy">
	...
	<a href={editHref} class="{accentColor} flex h-8 w-8 items-center justify-center rounded-full text-white">
...
<div class="divide-y divide-navy/8 rounded-2xl border border-navy/8">
	{#each menuItems as item}
		<a href={item.href} class="flex items-center gap-3 px-4 py-3.5">
			...
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4 shrink-0 text-navy/25">
