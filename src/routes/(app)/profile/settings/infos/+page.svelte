<script lang="ts">
	...
	// Champs communs
	let firstname = $state(initialUser.firstname ?? '');
	...
	// Previews photo
	let avatarPreview = $state<string | null>(initialUser.image ?? null);
	...
	$effect(() => {
		if (isChief && data.profile) {
			...
			bannerPreview = (data.profile as { banner_chief?: string | null }).banner_chief ?? null;
			specialties =
				data.profile.specialties?.map((s: { name_speciality: string }) => s.name_speciality) ?? [];
		...
		<label class="cursor-pointer">
			...
			{#if avatarPreview}
				<img
					src={avatarPreview}
					alt=""
					class="h-16 w-16 rounded-full object-cover ring-2 ring-cream ring-offset-2"
				/>
			{:else}
				<div
					class="flex h-16 w-16 items-center justify-center rounded-full bg-navy/10 text-navy/30"
				>
					<svg
	...
	{#if isChief}
		<div>
			...
			<label class="cursor-pointer">
				...
				{:else}
					<div
						class="flex h-16 w-28 items-center justify-center rounded-xl bg-navy/10 text-navy/30"
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							fill="none"
...
<div class="flex flex-col gap-4">
	<div>
		<p class="mb-1 block text-xs font-medium text-navy/70"
			>{isChief ? 'Nom et prénom du chef' : 'Nom et prénom'}</p
		>
	...
	<div>
		<label for="infos-bio" class="mb-1 block text-xs font-medium text-navy/70">Bio</label>
		<textarea
			id="infos-bio"
			bind:value={bio}
			rows="3"
			placeholder={isChief
				? 'Passionnée par la pâtisserie moderne, je réalise...'
				: "Passionné de gastronomie et avide d'expériences"}
			class="w-full resize-none rounded-xl border border-navy/15 bg-white px-3.5 py-2.5 text-sm text-navy outline-none placeholder:text-navy/30 focus:border-navy/40"
		></textarea>
	</div>
	...
	{#if isChief}
		...
		<div>
			<p class="mb-1 block text-xs font-medium text-navy/70"
				>Spécialités <span class="text-navy/40">(3 max)</span></p
			>
			<div class="relative">
				<button
					type="button"
				...
				{#if specialtyDropdownOpen}
					<ul
						class="absolute top-full left-0 z-20 mt-1 max-h-48 w-full overflow-y-auto rounded-xl border border-navy/10 bg-white shadow-lg"
					>
						{#each data.specialties as s (s.name_speciality)}
							<li>
								<button
									type="button"
									onclick={() => toggleSpecialty(s.name_speciality)}
									class="flex w-full items-center justify-between px-4 py-2.5 text-sm text-navy hover:bg-cream {specialties.includes(
										s.name_speciality
									)
										? 'font-semibold'
										: ''}"
			...
			{#if specialties.length > 0}
				<div class="mt-2 flex flex-wrap gap-2">
					{#each specialties as s (s)}
						<span
							class="flex items-center gap-1.5 rounded-full bg-teal/15 px-3 py-1 text-xs font-medium text-teal"
						>
							{s}
							<button
								type="button"
								onclick={() => removeSpecialty(s)}
								aria-label="Supprimer la spécialité {s}"
								class="text-teal/60 hover:text-teal"
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 16 16"
		...
		<!-- Villes -->
		<div>
			<p class="mb-0.5 block text-xs font-medium text-navy/70">Villes d'intervention</p>
			...
			<div class="relative">
				...
				{#if cityDropdownOpen}
					<div
						class="absolute top-full left-0 z-20 mt-1 w-full rounded-xl border border-navy/10 bg-white p-2 shadow-lg"
					>
						<div class="flex gap-2">
							<input
								type="text"
					...
					{#each cities as c (c)}
						...
						>
							...
							<button
								...
								aria-label="Supprimer la ville {c}"
								class="text-teal/60 hover:text-teal"
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
	...
	{:else}
		<!-- Ville (client) -->
		<div>
			<label for="infos-localization" class="mb-1 block text-xs font-medium text-navy/70">Ville</label>
			<input
				type="text"
				id="infos-localization"
...
{#if showConfirm}
	<div
		class="fixed inset-0 z-50 flex items-end justify-center bg-black/30 pb-6"
		role="button"
		tabindex="0"
		onclick={() => (showConfirm = false)}
		onkeydown={(e) => e.key === 'Enter' && (showConfirm = false)}
	>
		<div
		...
		>
			...
			<button
				...
				onclick={() => (showConfirm = false)}
				aria-label="Fermer"
				class="absolute top-4 right-4 text-navy/40 hover:text-navy"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 20 20"
			...
			<p class="mt-1 text-center text-sm text-navy/50">
				Vos informations de profil<br />seront mises à jour.
			</p>
			<button
				type="button"
				onclick={save}
			...
			<button
				type="button"
				onclick={() => (showConfirm = false)}
				class="mt-3 w-full py-2 text-sm text-navy/50"
			>
				Annuler
			</button>
		</div>
