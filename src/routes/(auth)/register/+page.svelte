<script lang="ts">
	...
	async function handleRegister(e: SubmitEvent) {
		...
		const result = await registerUser({ firstname, name, email, password, confirmPassword, localization, role });
...
<div class="flex min-h-dvh flex-col items-center bg-cream px-6 pt-10 pb-12">
	...
	<form onsubmit={handleRegister} class="flex w-full flex-col gap-4">
		<div class="flex flex-col gap-1">
			...
			<input type="text" bind:value={firstname} placeholder="ex : Marie" required
		...
		<div class="flex flex-col gap-1">
			...
			<input type="text" bind:value={name} placeholder="ex : Dupont" required
				class="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-navy" />
		...
		<div class="flex flex-col gap-1">
			...
			<input type="email" bind:value={email} placeholder="votre adresse mail" required
				class="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-navy" />
		...
		<div class="flex flex-col gap-1">
			...
			<input type="text" bind:value={localization} placeholder="votre ville" required
				class="rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-navy" />
